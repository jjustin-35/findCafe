        ...
        <form action="" className='searchbar container'>
          <div className="row justify-content-end">
            <div className="col offset-lg-1">
              <h2 className='fs-3-5 text text-white'>找到專屬於你的咖啡</h2>
              <div className="mb-1">
                <input type="text" placeholder='搜尋關鍵字' className='me-1 p-0-5 fs-1 rounded-2 w-35' onChange={(e)=>{setKeyword(e.target.value)} } value={ keyword } />
                <Select opt={countries} name='countries' id='' className='d-inline fs-1 py-0-5' onChange={ (e)=>{setLocate(e.target.value)} } />
              </div>
              
              <button className="btn btn-primary text-white d-block px-3 py-0-75 rounded-pill" onClick={handleSearch}>搜索</button>
            </div>
          </div>
        </form>
      </div>
