...
import { useGlobal } from '../../redux/global';
...
export const Find: React.FC = () => {
  ...
  const { areas } = useGlobal().address as { areas: Area[] };
  const { setSearch } = useGlobal().searchState as { setSearch: (query: any) => void };
  const [line, setLine] = useState<MrtLine[]>([]);
  const [station, setStation] = useState<string[]>([]);
  const [pages, setPages] = useState(0);
  const [nowPage, setNowPage] = useState(0);
  const countries = areas.map((area: Area) => area.name);
  countries.unshift('請選擇');

  useEffect(() => {
    (async () => {
      const response = await fetch(mrtApi);
      const mrtLine = await response.json();

      const formattedMrtLine = mrtLine.map((line: any) => ({
        name: line.LineName.Zh_tw,
        id: line.LineID,
      }));

      setLine(formattedMrtLine);
    })();
  }, []);

  const getStation = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const stationNames = stations[0].Stations.map((station: any) => station.StationName.Zh_tw);

    setStation(stationNames);
  ...
  const onSubmit = (data: any) => {
    ...
    // address
    let address: any = { country, districts, location, mrt: station };
