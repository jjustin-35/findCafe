          ...
          <section className="col-lg-8 col-12 mb-xl-0 mb-2">
            ...
            {comment.length != 0 ? <div>
              {comment.map((aComment) => {
                ...
                return (
                  <div>
                    {(!isEdit.id || !(isEdit.id === aComment._id)) ? <div className="d-flex justify-content-between align-items-start mb-1" key={aComment._id}>
                    ...
                    <div className="d-flex w-100 justify-content-between">
                      ...
                      <div>
                        ...
                        {aComment.user.email === profile.email && <div className=' text-end'>
                          <a href="" onClick={handleEdit}>編輯</a>｜
                          <a href="" onClick={handleDelete}>刪除</a>
                        </div>}
                      </div>
                    </div>
                    </div> : <EditComment method={"PATCH"} cafe={aComment.cafe} content={aComment.post} theStar={aComment.stars} theTags={aComment.tags} setIsEdit={setIsEdit} comment={comment} setComment={setComment} theId={ aComment._id } />}
                  </div>
                )
              })}
            </div> : <p className="fs-1 text-center text-light">留下第一筆留言!</p>}
            {/* myComment */}
            {!token ? <p className="fs-1-5 text-gray text-center">請先<Link to="/login" className="text-blue">登入</Link>再留言</p> : <EditComment method="POST" cafe={ theCafe } starSize="fs-1-5"/>}
          </section>
