...
export const EditComment = ({method, cafe, content = "", theStar, theTags, theId = undefined, starSize = "fs-1-25", setIsEdit, comment = [], setComment}) => {
  ...
  return (
      <form action="" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
            <a className={'bi bi-star-fill text-light hover hover-text-yellow ' + starSize + (stars >= 1 ? " text-yellow" : "") + (hoverStar >= 1 ? " text-yellow" : "")} onClick={handleStar} onMouseEnter={(e)=>{setHoverStar(e.target.id)}} id="1"></a>
            <a className={'bi bi-star-fill text-light hover hover-text-yellow ' + starSize + (stars >= 2 ? " text-yellow" : "") + (hoverStar >= 2 ? " text-yellow" : "")} onClick={handleStar} onMouseEnter={(e)=>{setHoverStar(e.target.id)}} id="2"></a>
            <a className={'bi bi-star-fill text-light hover hover-text-yellow ' + starSize + (stars >= 3 ? " text-yellow" : "") + (hoverStar >= 3 ? " text-yellow" : "")} onClick={handleStar} onMouseEnter={(e)=>{setHoverStar(e.target.id)}} id="3"></a>
            <a className={'bi bi-star-fill text-light hover hover-text-yellow ' + starSize + (stars >= 4 ? " text-yellow" : "") + (hoverStar >= 4 ? " text-yellow" : "")} onClick={handleStar} onMouseEnter={(e)=>{setHoverStar(e.target.id)}} id="4"></a>
            <a className={'bi bi-star-fill text-light hover hover-text-yellow ' + starSize + (stars >= 5 ? " text-yellow" : "") + (hoverStar >= 5 ? " text-yellow" : "")} onClick={handleStar} onMouseEnter={(e)=>{setHoverStar(e.target.id)}} id="5"></a>
        </div>
        </div>
        ...
        <textarea rows={5} placeholder='留言...' className='form-control' defaultValue={content} {...register('post',{
            required: {
      ...
      {method === "POST" ? <button className="btn btn-primary px-1-5 py-0-5">留言</button> : (
        <div>
          <button className="btn btn-primary px-1-5 py-0-5 mb-1 me-1">修改</button>
          <button className="btn btn-outline-primary px-1-5 py-0-5 mb-1" onClick={(e) => { e.preventDefault();setIsEdit({id: "", state: false}) }}>取消</button>
        </div>
      )}
    </form>
  )
