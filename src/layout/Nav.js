  ...
  if (!token) {
    return (
      <ul className="navbar-nav align-items-center">
        ...
        <li className="nav-item"><Link to='/signup' type="button" className="btn btn-white rounded-pill px-2 d-lg-block d-none" onClick={()=>{setErr("")}}>註冊</Link></li>
        <li className="nav-item d-lg-none">
          <Link to='/login' className='nav-link text-white fs-1-5 text-decoration-none' onClick={()=>{setErr("")}}>登入</Link>
        </li>
        <li className="nav-item d-lg-none">
          <Link to='/signup' className='nav-link text-white fs-1-5 text-decoration-none' onClick={()=>{setErr("")}}>註冊</Link>
        </li>
      </ul>
    )
  } else {
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-dark bg-primary sticky-top">
      <div className="container">
        <a href="" className="navbar-brand p-0 fs-2"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40" fill="none" className='me-0-5 align-middle nav-logo'>
        ...
        </svg><img src={`${localUrl}/img/findCafe.svg`} alt="" /></a>
